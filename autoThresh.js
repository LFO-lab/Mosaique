...
// Modification by Jean-Philippe Jullin, June 13, 2023 
...
function max_iter(v) {
	if (v<0){
		error("threshfinder: the maximum number of iterations should be positive\n");
		return;
	}
	max_iter = Math.floor(v);
}

function msg_int(v) {
	var dVal, dThresh, nThresh;
	if (iters > max_iter) { //if we reach the max number of iterations, stops and tell us so
		outlet(1,"failed");
		return;
		...
		if (Math.abs(target - v) <= tolerance){ //if we have reached the target within the tolerance then call it a victory
			...
			if (dVal != 0)  { // if there is a significan difference, make a line between the 2 and interpolates what would be a line towards the target
				nThresh = Math.max(0., Math.min(10000.,((dThresh / dVal) * (target - v)) + curThresh));
			} else { //otherwise, changes the threshold further in the same direction, to push for a change
				nThresh = Math.max(0., Math.min(10000.,dThresh + curThresh));
			}

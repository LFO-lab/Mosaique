...
function calculateAngleDifference(worldPoints, pointsA, pointsB) {
  ...
  var angle_difference = Math.acos(dot_product / mult_lengths); // Angular distance in radians
...
function rayPoints(aX, aY, aZ, bX, bY, bZ, numPoints) {
  ...
  // Get the closest numPoints points
  var closest_points = sorted_array.slice(0, numPoints);
  
  // Create a single array interleaving point ids and their angular distances
  var result = [];
  for (var i = 0; i < closest_points.length; i++) {
    result.push(closest_points[i][1], closest_points[i][0]);
  }
